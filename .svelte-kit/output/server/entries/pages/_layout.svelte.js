...
const Toc = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_page();
  return ` ${anchors2.length ? `<div class="${["toc svelte-1ulaavb", $tocCollapsed ? "collapsed" : ""].join(" ").trim()}"><div class="title svelte-1ulaavb">${escape(themeOptions?.i18n?.onThisPage)}</div> <div class="anchors svelte-1ulaavb"${add_attribute("style", `--bar-top: calc(${activeIdx * 2}em);`, 0)}>${each(anchors2, (an, i) => {
...
const MobileSubNav = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<nav class="sub-nav svelte-d6nxno" aria-label="Browse docs"><div role="button" tabindex="0" class="i-ic-round-menu-open text-6"></div> <div role="button" tabindex="0">${escape(themeOptions?.i18n?.onThisPage)}</div> </nav>`;
});
...
const GlobalLayout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    $$rendered = ` ${validate_component(Navbar, "Navbar").$$render($$result, {}, {}, {})} ${$page.error ? `${validate_component(Error, "Error").$$render($$result, { error: $page.error }, {}, {})}` : `<main class="svelte-tkaw86">${validate_component(AjaxBar, "AjaxBar").$$render(
      $$result,
      { this: ajaxBar },
      {
        this: ($$value) => {
